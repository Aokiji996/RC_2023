...
//            //将雷达测得的角度与实际角度向校准，得到雷达实际测得的柱的角度
    ...
    for(int i = 0; i < 2; i++){
        ...
        results[6+i].x = temper.x;
        results[6+i].y = temper.y;
        results[6+i].distance = temper.distance;
        std::copy(std::begin(temper.points), std::end(temper.points), std::begin(results[6+i].points));
    }
    return vis_cloud1;
}
